...
# DEFINITIVE FIX: Ensures no Streamlit UI calls are made before this, preventing StreamlitAPIException.
...
from utils.rag_utils import get_vector_store
...
# BEST PRACTICE: Use setdefault for cleaner initialization.
...
def create_agent(chat_model, vector_store, response_mode):
    ...
    if vector_store:
        ...
        doc_tool = Tool(
            ...
            description="Searches ONLY the uploaded document. Input must be a search query."
        )
        ...
        document_first_instruction = (
            ...
            "Analyze the results. If the information is sufficient, provide the answer based ONLY on the document. "
            "If, and only if, the document search is insufficient, then you may use the `tavily_search_results_json` tool."
        )

    prompt = hub.pull("hwchase17/react")
    mode_instructions = "Respond in a detailed, multi-paragraph format." if response_mode == "Detailed" else "Respond concisely, in 3 sentences or less."
...
def handle_chat_interaction(prompt, provider, model, temp, mode):
    ...
    with st.chat_message("assistant"):
        # DEFINITIVE FIX: Initialize `output` to prevent UnboundLocalError if an exception occurs before assignment.
        output = "I'm sorry, an error occurred. Please try again."
        try:
